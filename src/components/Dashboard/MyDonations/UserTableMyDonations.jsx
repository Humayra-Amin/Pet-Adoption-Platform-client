...
export default function UserTableMyDonations() {
    const { user } = useAuth();
        ...
        },
{
            ...
            cell: ({ row }) => (
                <img src={row.original.petImage} alt={row.original.petName} className="w-16 h-16 object-cover rounded-full" />
            ),
        },
{
            header: 'Pet Name',
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <div className="flex space-x-2">
                    <button
                        className="px-2 py-1 bg-pink-400 text-white rounded hover:bg-pink-300"
                            ...
                            {headerGroup.headers.map(header => (
                                <th key={header.id} onClick={header.column.getToggleSortingHandler()}>
                                    ...
                                    {{
                                        'asc': ' ⬆️',
                                        'desc': ' ⬇️',
                                    }[header.column.getIsSorted() ?? 'default'] || ' ↕️'}
                                </th>
                            ))}
                        </tr>
                    ))}
...
<button
                        ...
                        className="px-4 py-2 bg-black text-white rounded hover:bg-slate-700 disabled:opacity-50"
                    >
                        Previous Page
                    </button>
<button
